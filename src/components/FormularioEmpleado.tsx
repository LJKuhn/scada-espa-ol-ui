...
const FormularioEmpleado = ({
  ...
  const validateForm = () => {
    ...
    if (!formData.rango) {
      newErrors.rango = "Seleccione un rango";
    }
    if (!formData.fabricaAsignada) {
      newErrors.fabricaAsignada = "Seleccione una fábrica";
    }
      ...
      <DialogContent className="sm:max-w-md bg-card border-border">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-4">
          <div className="grid gap-4">
            ...
            {/* Rango */}
            <div className="space-y-2">
              <Label htmlFor="rango" className="text-foreground">
                Rango
              </Label>
              <Select
                value={formData.rango}
                onValueChange={(value) =>
                  setFormData({ ...formData, rango: value })
                }
              >
                <SelectTrigger
                  id="rango"
                  className={errors.rango ? "border-destructive" : ""}
                >
              ...
              {errors.rango && (
                <p className="text-xs text-destructive">{errors.rango}</p>
              )}
            </div>

            {/* Fábrica Asignada */}
            <div className="space-y-2">
              <Label htmlFor="fabrica" className="text-foreground">
                Fábrica Asignada
              </Label>
              <Select
                value={formData.fabricaAsignada}
                onValueChange={(value) =>
                  setFormData({ ...formData, fabricaAsignada: value })
                }
              >
                <SelectTrigger
                  id="fabrica"
                  className={errors.fabricaAsignada ? "border-destructive" : ""}
                >
              ...
              {errors.fabricaAsignada && (
                <p className="text-xs text-destructive">{errors.fabricaAsignada}</p>
              )}
            </div>
          </div>
